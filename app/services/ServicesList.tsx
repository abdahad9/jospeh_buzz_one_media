...
export default async function ServicesList() {
  ...
  const services = await sanity.fetch(`*[_type == "service"] | order(date desc){
    slug,
    title,
    icon{
      asset->{url}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 md:gap-0 pb-8 border-b border-gray-200">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-6 justify-center md:justify-start">
                  <Image src={service.icon.asset.url} alt={service.title} width={50} height={50} />
                ...
                <div className="flex flex-wrap gap-2 mb-4 md:pr-100 justify-center md:justify-start">
                  {service?.subServices?.map((tag: SubService) => (
                    <Link href={`/services/${service.slug.current}/${tag.subslug.current}`} key={tag.subslug.current}>
                      <span key={tag.subslug.current} className="inline-flex items-center bg-[#F1F3FF] text-[#031CA6] text-xs sm:text-sm font-light px-3 py-2 rounded-md whitespace-nowrap">
                        {tag.title}
                        {tag.subslug.current != "See More..." && <Image src="/images/arrow-up-right.svg" className="ml-2" alt="arrow-right" width={18} height={10} />}
                      </span>
                    </Link>
                  ))}
                </div>
              ...
              <div className="md:w-1/3 flex flex-col items-center md:items-start md:pl-26 text-center md:text-left">
                ...
                <Link href={`/services/${service.slug.current}`} className="inline-flex items-center text-[#031CA6] text-base sm:text-lg font-light group border-b-1 border-[#031CA6] pb-0.5 hover:border-blue-700 transition-all">
                  More Detail
