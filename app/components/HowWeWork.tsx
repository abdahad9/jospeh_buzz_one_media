            ...
            {steps.map((step) => (
              <div key={step.title} className="flex flex-col gap-2 items-center md:items-start text-center md:text-left w-full">
                <div className="flex flex-col md:flex-row items-center md:items-center gap-2 md:gap-4 w-full">
                  <span className="w-15 h-15 flex items-center justify-center rounded-2xl bg-[#FAD99B] text-[#C27F04] font-medium text-3xl mb-2 md:mb-0">{steps.indexOf(step) + 1}</span>
                ...
                {steps.indexOf(step) < steps.length && <div className="w-full border-t border-[#F7C569] mt-4" />}
              </div>
            ))}
          </div>
        </div>
